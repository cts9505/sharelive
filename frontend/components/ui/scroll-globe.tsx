...
import { Rocket, Plus, Zap, Globe as GlobeIcon, Shield, ArrowRight } from "lucide-react";
...
interface ScrollGlobeProps {
  sections: {
    ...
    actions?: { label: string; variant: 'primary' | 'secondary'; href?: string; onClick?: () => void; icon?: React.ReactNode }[];
  }[];
  globeConfig?: {
    positions: {
      top: string;
      ...
      {sections.map((section, index) => (
        ...
        >
          <div className={cn(
            ...
            {/* Badge */}
            {section.badge && (
              <p className="text-xs sm:text-sm uppercase tracking-[0.3em] text-primary mb-4 sm:mb-6 font-medium">
                {section.badge}
              </p>
            )}
            
            <h1 className={cn(
              "font-bold mb-4 sm:mb-6 lg:mb-8 leading-[1.1] tracking-tight",
              index === 0 
            ...
            <div className={cn(
              "text-muted-foreground/80 leading-relaxed mb-6 sm:mb-8 lg:mb-10 text-sm sm:text-base lg:text-lg xl:text-xl font-light",
              ...
              {section.highlights && section.highlights.length > 0 && (
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm text-muted-foreground/60 mt-4 sm:mt-6">
                  {section.highlights.map((highlight, highlightIndex) => (
                    <div key={highlight} className="flex items-center gap-1.5 sm:gap-2">
                      <div
                        className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse"
                        style={{ animationDelay: `${highlightIndex * 0.4}s` }}
                      />
                      <span>{highlight}</span>
                    </div>
                  ))}
            ...
            {/* Enhanced Features */}
            {section.features && (
              <div className="grid gap-3 sm:gap-4 mb-6 sm:mb-8 lg:mb-10">
                {section.features.map((feature, featureIndex) => (
                  <div 
                    key={feature.title}
                    className={cn(
                      "group p-4 sm:p-5 lg:p-6 rounded-xl border bg-card/50 backdrop-blur-sm hover:bg-card/80 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5",
                      "hover:border-primary/20 hover:-translate-y-1"
                    )}
                    style={{ animationDelay: `${featureIndex * 0.1}s` }}
                  >
                    <div className="flex items-start gap-3 sm:gap-4">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary/20 transition-colors flex-shrink-0">
                        {feature.icon || <Zap className="w-4 h-4 sm:w-5 sm:h-5" />}
                      </div>
                      <div className="flex-1 space-y-1 sm:space-y-1.5 min-w-0">
                        <h3 className="font-semibold text-card-foreground text-sm sm:text-base lg:text-lg">{feature.title}</h3>
                        <p className="text-muted-foreground/80 leading-relaxed text-xs sm:text-sm lg:text-base">{feature.description}</p>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {section.actions.map((action, actionIndex) => (
                  action.href ? (
                    <Link
                      ...
                      href={action.href}
                      className={cn(
                        "group relative inline-flex items-center justify-center gap-2 px-5 sm:px-6 lg:px-8 py-2.5 sm:py-3 lg:py-4 rounded-full font-medium transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] text-sm sm:text-base",
